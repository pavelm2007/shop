    ...
    def get_context(self, context, object, button_text, added_text, already_in_text, template_name):
        ...
        if object is None:
            ...
            try:
                ...
                already_in_qty = OrderItem.objects.filter(order=order_id, content_type=content_type, object_id=object.id).exclude(order__status=3).count()
...
class AddConfirmBasketButton(InclusionTag):
    name = 'add_confirm_basket_button'
    template = 'basket/button_confirm.html'
    ...
    def get_template(self, context, object, button_text, template_name):
        if template_name:
            template_to_render = template_name
        else:
            template_to_render = self.template
    ...
    def get_context(self, context, object, button_text, template_name):
        context_to_return = {}
        if object is None:
            context_to_return.update({
                'content_type': None,
...
register.tag(AddConfirmBasketButton)
