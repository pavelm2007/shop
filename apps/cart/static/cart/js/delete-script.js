...
function updateElementIndex(el, prefix, ndx) {
    ...
    $(document).bind("before_del.basket", function(e){
        if (cart.num_forms.val() === 1) {
            cart.do_delete = confirm("{% trans 'Are you sure about deleting last item?' %}");
       }
    });

    var prefix = '{{ formset.prefix }}';
    cart.num_forms = $('#id_'+prefix+'-TOTAL_FORMS');
    ...
    $(document).bind("del_success.basket", function(e, data){
        ...
        var formCount = parseInt(cart.num_forms.val(), 10);
        cart.num_forms.val(forms.length);
